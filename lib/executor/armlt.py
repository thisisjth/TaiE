...
from lib.stack.armlt import ArmLTStack
...
class ArmLTExecutor(Executor):
    ...
    # 条件跳转指令
    cjmps:List[str] = Helper.merge(*map(lambda b: Helper.merge(*list(map(lambda c:[b+c, b+c+'.w'], conds[:-3]))), ['b']))
    # 非条件跳转指令
    ...
    stack_op_regs:Dict[str, Dict[str, Tuple[Pattern, int]]] = {
        ...
        'O': {
            ...
            # thumb: push {rrr, lr}  push {lr}
            'push_lr_thumb': (re.compile(rb'.{1}\xB5'), 2),
            # thumb: push.w
            'push_lr_thumb_w': (re.compile(rb'[\xAD\x2D]\xE9.{1}[\x40\x41\x42\x44\x45\x48\x4F\x50]'), 2),
            # push {lr}
    ...
    logger:Logger = Logger('Executor.ArmLT')
    ...
    def search_stack_ops(cls, bin:Binary) -> List[Tuple[int, bytes, int, str, str]]:
        ...
        return list(filter(lambda r:(bin.text_base+r[0]) % (2 if 'thumb' in r[3] else 4) == 0, results))
    ...
    def reliable_stack_op_groups(cls, stack_ops:List[Tuple[int, bytes, int, str, str]], bin:Binary, l:int=2) -> List[Tuple[int, int]]:
        ...
        while i < len(op_groups):
            ...
            the_addr:int = 2 * ((bin.text_base+stack_ops[grp[0]][0]) // 2)
            try:
    ...
    def __idx_reg_boundary(cls, idx_reg:str, bin:Binary, history:List[CsInsn]) -> int:
        ...
        '''
        # 寻找对索引寄存器进行比较的指令
        the_cmp_insn_idx:int = Helper.last_index(lambda i:i.mnemonic in ['cmp', 'cmp.w'] and i.op_str.split(',')[0] == idx_reg, history)
        # 如果找不到cmp指令 那就没法分析了
        if the_cmp_insn_idx < 0: 
            cls.logger.error(idx_reg, history)
            raise InsnNotFound('Cannot find a valid cmp instruction')
        # 分析比较的对象是谁
        ...
        try:
            for insn in insns:
                ...
                if insn.mnemonic.startswith('stm'):
                    ...
                    full_or_empty:str = ['e', 'e', 'e', 'e', 'f', 'f', 'f', 'f', 'f'][['ea', 'ia', 'ed', 'da', 'fa', 'ib', 'fd', 'db', 'tm'].index(insn.mnemonic[-2:])]
                    ace_or_desc:str = ['a', 'a', 'd', 'd', 'a', 'a', 'd', 'd', 'd'][['ea', 'ia', 'ed', 'da', 'fa', 'ib', 'fd', 'db', 'tm'].index(insn.mnemonic[-2:])]
                    ...
                    values:List[str] = ops[1].strip('{').strip('}').strip().split(', ')
                    full_or_empty:str = ['e', 'f', 'e', 'f', 'f', 'e', 'f', 'e', 'f'][['ea', 'ia', 'ed', 'da', 'fa', 'ib', 'fd', 'db', 'dm'].index(insn.mnemonic[-2:])]
                    ace_or_desc:str = ['a', 'd', 'd', 'a', 'a', 'd', 'd', 'a', 'd'][['ea', 'ia', 'ed', 'da', 'fa', 'ib', 'fd', 'db', 'dm'].index(insn.mnemonic[-2:])]
                ...
                if 'sp' not in insn.op_str and 'fp' not in insn.op_str: continue
                if insn.mnemonic.startswith('mov'): s.mov(ops[1].strip(), ops[0].strip())
    ...
    def gen_exe_paths_from_addr(cls, head:int, bin:Binary, min_addr:int=-1, max_addr:int=math.inf, funcs_map:Dict[int, Func]={}, ignore_indirect_call:bool=True, extend_on_demand:bool=True, use_cache:bool=True, from_func_head:bool=False) -> List[ExePath]:
        ...
        # 如果反汇编起始地址不是对齐 则没法搞了
        if head % 2 != 0: 
            # raise InvalidAddress(f'Instructions should be aligned with 2 bytes: {hex(head)}')
            cls.logger.error(f'Instructions should be aligned with 2 bytes: {hex(head)}')
            return exe_paths
        try:
            ...
            return exe_paths
        # 待分析队列
        states:Queue[Tuple[List[CsInsn], int, List[int], List[int], List[int], List[CsInsn], int, int]] = Queue()
        # 已加入队列的情况
        done_states:List[int] = [head]
        states.put_nowait(([], head, [], [], [], insns, 0, 0,))
        while not states.empty():
            history, addr, ucjmps, callees, constants, insns, it_then_count,it_else_count = states.get_nowait()
            states.task_done()
            # 是否是为了避免循环而结束的分析
            ...
            while True:
                    ...
                    if Helper.is_int(insn.op_str) or (insn.mnemonic in ['cbz', 'cbnz'] and Helper.is_int(insn.op_str.split(',')[1])):
                        ...
                        try:
                            ...
                            if jmp_target not in done_states:
                                # 那就加入待分析队列吧
                                states.put_nowait((history.copy(), jmp_target, ucjmps.copy(), callees.copy(), constants.copy(), tgt_insns, 0, 0,))
                ...
                # 函数返回指令
                    ...
                    # 如果是有条件返回 就需要分叉执行路径了
                    if it_then_count <= 0 and cls.has_cond(insn):
                        ...
                        try:
                            ...
                            if tgt_addr not in done_states: 
                                states.put_nowait((history.copy(), tgt_addr, ucjmps.copy(), callees.copy(), constants.copy(), tgt_insns, 0, 0,))
                ...
                # 函数调用指令
                    ...
                    if Helper.is_int(insn.op_str):
                        ...
                        try:
                            ...
                            if len(tmp_insns) >= 3 and cls.null_insn(tmp_insns[1], bin) and cls.null_insn(tmp_insns[2], bin): break
                        except Exception as e:
                            cls.logger.error(e)
                ...
                # arm特有 ldr指令处理
                    ...
                    constant:int = Helper.align(Helper.align(insn.address)+(4 if insn.size == 2 or insn.mnemonic.find('.w') > 0 else 8))+Helper.to_int(insn.op_str.split(',')[2].replace(']', ''))
                    constants.append(constant)
                # it指令处理 
                elif cls.is_it(insn): 
                    # 计算else语句块距离多远
                    insns_to_else:int = insn.mnemonic[1:].count('t')+1
                    # 计算else语句块有多少条指令
                    insns_in_else:int = insn.mnemonic.count('e')
                    # 如果超过了现有的指令 就需要拓展
                    if index+insns_to_else >= len(insns): 
                        try:
                            insns.extend(InsnMgr.insns_from(insns[-1].address+insns[-1].size, bin, cls))
                        except Exception as e:
                            cls.logger.error(e)
                            ...
                            break
                    # 将else分支加入待处理队列
                    states.put_nowait((history.copy(), insns[index+insns_to_else].address, ucjmps.copy(), callees.copy(), constants.copy(), insns[index+insns_to_else:], 0, 0))
                    done_states.append(insns[index+insns_to_else].address)
                    # 将then分支加入待处理队列
                    states.put_nowait((history.copy(), insns[index+1].address, ucjmps.copy(), callees.copy(), constants.copy(), insns[index+1:], insn.mnemonic[1:].count('t'), insns_in_else))
                    done_states.append(insns[index+1].address)
                    break
                it_extra_offset:int = 0
                # 如果是从it指令跳过来的
                if it_then_count > 0:
                    it_then_count -= 1
                    # 如果当前指令执行完 就需要结束it指令块了 那就进行跳转
                    if it_then_count == 0: it_extra_offset = it_else_count
                index += 1 + it_extra_offset
        ...
        if use_cache:
            ...
            for exe_path in exe_paths:
                # 将新的分析结果进行缓存
                addrs:List[int] = list(set(map(lambda insn: insn.address, exe_path.insns)))
    ...
    def is_ucjmp(cls, insn:CsInsn, last_insn:CsInsn=None) -> bool:
        ...
        return (
            ...
            or (insn.mnemonic in ['b.w', 'tbb', 'tbb.w', 'tbh', 'tbh.w'])
        )
